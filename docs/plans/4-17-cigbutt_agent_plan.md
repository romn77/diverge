...
对应的代码面，至少会改这些文件。`diverge/agents/analysts/cigbutt_analyst.py` 放 agent 本体；`diverge/agents/__init__.py` 导出 `create_cigbutt_analyst`；`diverge/agents/utils/agent_states.py` 和 `diverge/graph/propagation.py` 加独立 state；`diverge/graph/conditional_logic.py` 加 `should_continue_cigbutt()`；`diverge/graph/trading_graph.py` 加 `tools_cigbutt` 和 state log；`diverge/graph/setup.py` 接入 sidecar agent 构图；`cli/models.py`、`cli/utils.py`、`cli/main.py`、`diverge/runner.py` 增加独立选择槽位、进度显示、报告 section 和落盘路径。现有 `agents/__init__.py` 现在只导出四个 analyst factory，所以这一步是必需的。: contentReference[oaicite: 6]{index=6}
