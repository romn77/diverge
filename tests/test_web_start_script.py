...
class WebStartScriptTests(unittest.TestCase):
    ...
    def test_start_script_supports_redis_worker_mode(self):
        ...
        self.assertIn(
            'export TASK_GLOBAL_RUNNING_LIMIT="$TASK_GLOBAL_RUNNING_LIMIT"', source
        )
        self.assertIn(
            'export TASK_USER_RUNNING_LIMIT="$TASK_USER_RUNNING_LIMIT"', source
        )
        self.assertIn(
        ...
        self.assertIn(
            "Running limits: global=$TASK_GLOBAL_RUNNING_LIMIT user=$TASK_USER_RUNNING_LIMIT",
            source,
        )
    ...
    def test_start_script_exposes_and_streams_runtime_logs(self):
        ...
        self.assertIn(
            'BACKEND_LOG_LEVEL="${BACKEND_LOG_LEVEL:-${LOG_LEVEL:-info}}"', source
        )
    ...
    def test_start_script_cleans_up_when_auth_bootstrap_fails_before_backend_pid_exists(
        self,
    ):
        script = PROJECT_ROOT / "web" / "start.sh"
        ...
        self.assertIn(
            "simulated alembic failure: postgres unavailable", combined_output
        )
