...
def resolve_screener_data_sources() -> dict[str, object]:
...
def build_screener_config_payload(request_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        return screener_preparation.build_screener_config_payload(
            request_payload,
            data_source_resolver=lambda markets: resolve_screener_data_sources(),
            us_manifest_error=(
...
def build_ohlcv_sync_payload(market: str, as_of_date: str) -> dict[str, Any]:
    ...
    payload.update(resolve_screener_data_sources())
    if market == "cn":
